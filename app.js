...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Entrance loading screen + clock
  const loadingScreen = $("#loadingScreen");
  if (loadingScreen) {
    window.setTimeout(() => loadingScreen.classList.add("is-done"), 900);
  }

  function updateClassroomClock(){
    const h = $("#clockHour"), m = $("#clockMinute"), sec = $("#clockSecond");
    if(!h || !m || !sec) return;
    const now = new Date();
    const seconds = now.getSeconds() + now.getMilliseconds()/1000;
    const minutes = now.getMinutes() + seconds/60;
    const hours = (now.getHours()%12) + minutes/60;
    h.style.transform = `translateX(-50%) rotate(${hours*30}deg)`;
    m.style.transform = `translateX(-50%) rotate(${minutes*6}deg)`;
    sec.style.transform = `translateX(-50%) rotate(${seconds*6}deg)`;
  }
  updateClassroomClock();
  if($("#clockSecond")) window.setInterval(updateClassroomClock, 1000);
  ...
  if (enter) {
    enter.addEventListener("click", () => {
      ...
      setTimeout(() => location.href = "classroom.html", 850);
    });
  }
